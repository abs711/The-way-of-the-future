...
class CycleEncoderDecoder(nn.Module):
    ...
    def __init__(self, model_params):
        ...
        self.encoder = CycleEncoder(model_params)
        self.decoder = CycleDecoder(model_params)
    ...
    def train_model(self, train_dict, n_epochs, training_prediction = 'recursive', teacher_forcing_ratio = 0.5, learning_rate = 0.01, dynamic_tf = False):
        ...
        input_tensor=train_dict['x']
        target_tensor=train_dict['y']
        ...
        with trange(n_epochs) as tr:
            for it in tr:
                ...
                for b in range(n_batches):
                    ...
                    # outputs tensor
                    outputs = torch.zeros((self.target_len, self.batch_size, self.output_dim))
...
#                    decoder_input = input_batch[0, :, :]   # shape: (batch_size, input_size)
                    decoder_input = torch.rand((self.batch_size,self.output_dim))  # shape: (batch_size, input_size)
    ...
    def predict(self, input_tensor):
        ...
        with torch.no_grad():
            ...
            outputs = torch.zeros(self.target_len, self.output_dim)
    
            # decode input_tensor
    #        decoder_input = input_tensor[-1, :, :]
            decoder_input = torch.rand((1,self.output_dim))  # shape: (batch_size, output_size)
...
#        
        lstm_out, self.hidden = self.enc_lstm(x_input.view(x_input.shape[0], x_input.shape[1], self.num_features))
...
class CycleDecoder(nn.Module):
    def __init__(self, model_params):
        ...
        self.dec_lstm = nn.LSTM(self.output_dim, self.hidden_dim,self.num_layers)
...
#        self.linear = nn.Linear(hidden_size, input_size)           
    ...
    def forward(self, x_input, encoder_hidden_states):
        ...
        lstm_out, self.hidden = self.dec_lstm(x_input.unsqueeze(0), encoder_hidden_states)
        output = self.dec_output_FC(lstm_out.squeeze(0))     
