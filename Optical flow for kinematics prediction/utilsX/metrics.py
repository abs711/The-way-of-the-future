...
class SequenceMultiClassMetric(BaseMetric):
    ...
    def record_output(self, output, target, batch_size=1):#output, output_indices, target, prev_absolutes, next_absolutes, batch_size=1):
        assert output.dim() == 4
        assert target.dim() == 3
...
##        print(output.size())
        ...
        assert num_class == self.args['num_classes'], "num_classes = "+str(num_class)
##        print('num_label: ', num_label,'num_class: ',num_class)
        ...
        for seq_id in range(sequence_length):
            for imu_id in range(num_label):
                ...
                imu_target = imu_target.long()
                imu_preds = predictions[:, seq_id, imu_id].contiguous()
                imu_preds = imu_preds.long()
                for label, pred in zip(imu_target.view(-1), imu_preds.view(-1)):
##                    print(seq_id, imu_id, label.dtype, pred.dtype)
                    self.confusion[seq_id, imu_id, label-1, pred] += 1.0
    ...
    def final_report(self):
        ...
        report += '   Accuracy Matrix (seq x imu x label): {}'.format(
            correct_percentage)
        return report, correct_percentage, self.confusion
