function [] = generate_character(npose,segments,character_root,rotation_vectors,camera_placement,idx)
%% Dimensions
hip_width = 26;
hip_height = 93;
knee_height = 48;
...
pelv_ind = find(strcmp(segments,'Pelvis'));
pelv_vec = rotate_vec_using_quat(pelv_vec,rotation_vectors(pelv_ind));

%% Upper Legs
Thigh_length = hip_height-knee_height; 

uLegR_vec = -[0, 0, Thigh_length];
uLegR_ind = find(strcmp(segments,'RightUpperLeg'));
uLegR_vec = rotate_vec_using_quat(uLegR_vec,rotation_vectors(uLegR_ind));

uLegL_vec = -[0, 0, Thigh_length];
uLegL_ind = find(strcmp(segments,'LeftUpperLeg'));
uLegL_vec = rotate_vec_using_quat(uLegL_vec,rotation_vectors(uLegL_ind));

%% Lower Legs
lLegR_vec = -[0, 0, knee_height];
lLegR_ind = find(strcmp(segments,'RightLowerLeg'));
lLegR_vec = rotate_vec_using_quat(lLegR_vec,rotation_vectors(lLegR_ind));

lLegL_vec = -[0, 0, knee_height];
lLegL_ind = find(strcmp(segments,'LeftLowerLeg'));
lLegL_vec = rotate_vec_using_quat(lLegL_vec,rotation_vectors(lLegL_ind));

%% Feet
FootR_vec = [Foot_length, 0, 0];
FootR_ind = find(strcmp(segments,'RightFoot'));
FootR_vec = rotate_vec_using_quat(FootR_vec,rotation_vectors(FootR_ind));

FootL_vec = [Foot_length,0, 0];
FootL_ind = find(strcmp(segments,'LeftFoot'));
FootL_vec = rotate_vec_using_quat(FootL_vec,rotation_vectors(FootL_ind));

%% Balls
BallR_vec = [Foot_length*ball_size, 0, 0];
BallR_ind = find(strcmp(segments,'RightToe'));
BallR_vec = rotate_vec_using_quat(BallR_vec,rotation_vectors(BallR_ind));
BallL_vec = [Foot_length*ball_size, 0, 0];
BallL_ind = find(strcmp(segments,'LeftToe'));
BallL_vec = rotate_vec_using_quat(BallL_vec,rotation_vectors(BallL_ind));

%% Spine
spine_length = shoulder_height-hip_height;

L5_vec = [0 0 0.25*spine_length];
L5_ind = find(strcmp(segments,'L5'));
L5_vec = rotate_vec_using_quat(L5_vec,rotation_vectors(L5_ind));

L3_vec = [0 0 0.25*spine_length];
L3_ind = find(strcmp(segments,'L3'));
L3_vec = rotate_vec_using_quat(L3_vec,rotation_vectors(L3_ind));

T12_vec = [0 0 0.25*spine_length];
T12_ind = find(strcmp(segments,'T12'));
T12_vec = rotate_vec_using_quat(T12_vec,rotation_vectors(T12_ind));

T8_vec = [0 0 0.25*spine_length];
T8_ind = find(strcmp(segments,'T8'));
T8_vec = rotate_vec_using_quat(T8_vec,rotation_vectors(T8_ind));

%% Shoulder-Neck-Head
over_shoulder = height-shoulder_height;

ShoulderR_vec = -[0 0.5*shoulder_width 0];
ShoulderR_ind = find(strcmp(segments,'RightShoulder'));
ShoulderR_vec = rotate_vec_using_quat(ShoulderR_vec,rotation_vectors(ShoulderR_ind));

ShoulderL_vec = [0 0.5*shoulder_width 0];
ShoulderL_ind = find(strcmp(segments,'LeftShoulder'));
ShoulderL_vec = rotate_vec_using_quat(ShoulderL_vec,rotation_vectors(ShoulderL_ind));

neck_vec = [0 0 neck_head_ratio*over_shoulder];
neck_ind = find(strcmp(segments,'Neck'));
neck_vec = rotate_vec_using_quat(neck_vec,rotation_vectors(neck_ind));

head_vec = [0 0 (1-neck_head_ratio)*over_shoulder];
head_ind = find(strcmp(segments,'Head'));
head_vec = rotate_vec_using_quat(head_vec,rotation_vectors(head_ind));
...
uArmR_ind = find(strcmp(segments,'RightUpperArm'));
uArmR_vec = rotate_vec_using_quat(uArmR_vec,rotation_vectors(uArmR_ind));
...
uArmL_ind = find(strcmp(segments,'LeftUpperArm'));
uArmL_vec = rotate_vec_using_quat(uArmL_vec,rotation_vectors(uArmL_ind));
...
lArmR_ind = find(strcmp(segments,'RightForeArm'));
lArmR_vec = rotate_vec_using_quat(lArmR_vec,rotation_vectors(lArmR_ind));
...
lArmL_ind = find(strcmp(segments,'LeftForeArm'));
lArmL_vec = rotate_vec_using_quat(lArmL_vec,rotation_vectors(lArmL_ind));
...
handR_ind = find(strcmp(segments,'RightHand'));
handR_vec = rotate_vec_using_quat(handR_vec,rotation_vectors(handR_ind));
...
handL_ind = find(strcmp(segments,'LeftHand'));
handL_vec = rotate_vec_using_quat(handL_vec,rotation_vectors(handL_ind));
%% Bases info
...
figure_properties
% view(rotate_vec_using_quat(camera_placement,rotation_vectors(pelv_ind)))
view(CameraLoc)
